...
- **Missing dependencies**: `utils/fast_OLS.py` needs `plotly` and `utils/mutual_inf_plot.py` needs `scikit-learn`; neither is installed in `.venv`, so importing those modules fails today. Available: numpy, pandas, seaborn, matplotlib, scipy, statsmodels, torch, tqdm, IPython and ipywidgets (the latter only for `domain_utils/visualizer_3d.py`). No scikit-image either, which is why that viewer draws the mask surface as a point cloud rather than a marching-cubes mesh.
- **`requirements.txt` is a `pip freeze` saved as UTF-16LE with BOM and CRLF** (PowerShell's default redirect), so git diffs it as binary and `Bin` is all you see in `--stat`. Keep that encoding when editing it — rewriting as UTF-8 would turn a two-line change into a whole-file diff. It also pins `torch==2.13.0+cu132`, a CUDA build that plain `pip install -r` cannot resolve without the PyTorch index URL. `requirements/dev.txt` covers lint tooling only.
